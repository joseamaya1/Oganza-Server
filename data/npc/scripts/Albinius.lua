...
local items = {
          item1 = {12437, 50731}, -- item e storage dado
          item2 = {19742, 50732}, -- item e storage dado
          item3 = {11213, 50733}, -- item e storage dado
          item4 = {2667, 50734}, -- item e storage dado
          item5 = {21246, 50735}, -- item e storage dado
	      item6 = {2676, 50736}, -- item e storage dado
	...
	local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
	    ...
	    if msgcontains(msg, 'death portal') then
		  ...
		  if player:getStorageValue(cid, items.item3[2]) <= 0 and getPlayerItemCount(cid, items.item3[1]) >= counts.count1[1] then
             ...
             elseif msgcontains(msg, 'fire portal') then
		  ...
		  if player:getStorageValue(cid, items.item6[2]) <= 0 and getPlayerItemCount(cid, items.item6[1]) >= counts.count1[1] then
                              doPlayerRemoveItem(cid, items.item6[1], counts.count1[1])
                              selfSay('Excellente! Now you can pass in the teleport.', cid)
							 player:setStorageValue(items.item6[2], counts.count1[2])
                    else
                              selfSay('You need '.. counts.count1[1] ..' '.. getItemName(items.item6[1]) ..'.', cid)
                    end
